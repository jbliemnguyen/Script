...
. "$env:dp0\..\Install\spFarmFunctions.ps1"
. "$env:dp0\..\Install\spServiceFunctions.ps1"
...
try {
    if ($databaseName -eq $nul -or $databaseName -eq '') {
        throw "Missing databaseName parameter";
    }
    SP-RegisterPS;
    $global:summaryNode = $global:data.Scan.SelectSingleNode("Summary");
    ProcessContentDB -name $databaseName;
    $global:data.Save($env:dp0 + "\$($databaseName)_scanresults.xml");
}
catch {
